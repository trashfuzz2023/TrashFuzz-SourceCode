...
async def hello(scenario_msg, single_spec, generation_number=1, population_size=3, max_obs_num=5, directory=None) -> object:
    # print(single_spec)
        ...
        if msg['TYPE'] == 'READY_FOR_NEW_TEST' and msg['DATA']:
            ...
            if len(population) < population_size: # get the initial population (random)
                ...
                for kk in range(len(testcase.cases)):
                    scenario_msg.append(testcase.cases[kk])
...
def spec_scenario(spec, testcase, generations=0, pop_size=1, max_obs_num=5, file_directory=None):
    ...
    loop.run_until_complete(
        asyncio.gather(hello(msgs, scenario_specification, generation_number=generations, population_size=pop_size, max_obs_num = max_obs_num,
    ...
    for i in range(len(origin_case)):
        ...
        try:
            ...
            for spec_index in range(len(specifications_in_scenario)):
                ...
                spec_scenario(spec=single_specification, testcase=test_case, generations=10, pop_size=30, max_obs_num = num2, file_directory=log_direct)
                # print(testcase)
        except KeyError:
