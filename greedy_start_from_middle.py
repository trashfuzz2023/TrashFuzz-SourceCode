...
async def hello(scenario_msg, single_spec, max_num_of_cases = 100, max_obs_num=5, directory=None) -> object:
	...
	remaining_sub_violations = []

	with open("remaining_list2.txt", "r") as filex:
		for line in filex:
			remaining_sub_violations.append(line.strip())
	...
	async with websockets.connect(uri,  max_size= 300000000, ping_interval=None) as websocket:
		...
		if msg['TYPE'] == 'READY_FOR_NEW_TEST' and msg['DATA']:
			...
			initial_scenario = scenario_msg[0]
			scenario_msg = []
			scenario_msg.append(initial_scenario)
			...
			while i < max_num_of_cases:
				...
				while True:
					...
					if msg['TYPE'] == 'READY_FOR_NEW_TEST':
						...
						with open("remaining_list2.txt", "w") as filex:
...
def test_scenario(input_file, testcase_direct, num2):
	file = input_file

	with open(testcase_direct) as json_file:
		file0 = json.load(json_file)

	log_direct ='/data/Attack-Of-ADS/Greedy/Apollo7.0.0/'+'num_'+str(num2)+ "_3_1"
...
if __name__ == "__main__":
	...
	testcase = 'Cases/temp.json'

	test_scenario(input_file, testcase, 4)
